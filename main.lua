...
local G = {
  ...
  -- initials entry
  initialsBuffer="", initialsTargetGs="", pendingScore=nil, pendingTime=nil,
  -- player
...
local function delayedEnd(gs)
  ...
  -- Route through initials entry when score or time qualifies
  local pendingGs    = gs
  local scoreQual    = (gs == "gameover" or gs == "win") and LB.qualifies(G.score)
  local timeQual     = gs == "win" and G.pendingTime ~= nil
  if scoreQual or timeQual then
    if scoreQual then
...
local function checkWin()
  ...
  if won then
    if G.level >= #LEVELS then
      if G.score > G.hi then G.hi = G.score end
      if LB.qualifiesTime(G.gameTime) then
        G.pendingTime = {time=G.gameTime}
      end
      delayedEnd("win")
    else
...
function love.keypressed(key)
  ...
  if G.gs == "initials" then
    ...
    elseif key == "return" or key == "space" then
      ...
      if G.pendingTime then
        G.pendingTime.initials = initials
        G.lastGameTimeRank = LB.insert(LB.data.time_game, G.pendingTime, function(a,b) return a.time < b.time end)
        G.pendingTime = nil
      end
      LB.save()
