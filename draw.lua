  ...
  if G.gs == "win" then
    ...
    if #LB.data.time_game == 0 then
      ...
      for i, e in ipairs(LB.data.time_game) do
        ...
        centeredAt(G.fnt, string.format("%d.  %s", i, fmtTime(e.time)), rcx, G.H/2+10+i*18, col)
      end
    end
    setCol(COL.sep)
...
function Draw.initials()
  ...
  centered(G.bfnt, "NEW  HIGH  SCORE!", G.H/2 - 148, COL.wordTyped)
  centered(G.fnt,  string.format("SCORE  %d   —   LEVEL %d",
    G.pendingScore.score, G.pendingScore.level), G.H/2 - 98, COL.hud)
...
function Draw.scores()
  ...
  if #LB.data.score > 0 then
    ...
    for i, e in ipairs(LB.data.score) do
      ...
      local txt = string.format("%2d. %-3s  %d  lv%d",
  ...
  if hasTimes then
    ...
    for i, e in ipairs(LB.data.time_game) do
      ...
      centeredAt(G.pfnt, fmtTime(e.time), tcx, y1 + (i-1)*rowH, c)
    end
    setCol(COL.sep)
